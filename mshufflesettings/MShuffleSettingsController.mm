...
@interface MShuffleSettingsController : PSListController {
	UILabel* _label;
	UILabel* underLabel;
}
- (void)HeaderCell;
...
@implementation MShuffleSettingsController
- (id)specifiers {
	if (!_specifiers) {
        ...
        [specifiers addObject:spec];
		spec = [PSSpecifier preferenceSpecifierNamed:@"When Album End"
                                              target:self
											  set:@selector(setPreferenceValue:specifier:)
											  get:@selector(readPreferenceValue:)
...
- (void)love
{
	...
	[twitter setInitialText:@"#MShuffle by @ijulioverne is cool!"];
	if (twitter != nil) {
...
- (void)reset
{
	...
	notify_post("com.julioverne.mshuffle/Settings");
}
...
- (void)HeaderCell
{
	...
	CGRect frame = CGRectMake(0, 20, width, 60);
		...
		[underLabel setText:@"Album End Play To Shuffle"];
